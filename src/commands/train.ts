   ...
   (client, message) => {
      ...
      if (!targetCategory)
         return client.room.sendMessage(
            'Veuillez spécifier une catégorie d\'entraînement, ou "off".',
            'danger'
         );
      ...
      if (targetCategory === 'off') {
         if (!client.room.trainCategory)
            return client.room.sendMessage(
               'Le mode entraînement est déjà inactif.',
               'danger'
            );
         ...
         client.room.sendMessage(
            'Le mode entraînement a été désactivé, les scores seront de nouveau enregistrés.',
            'success'
         );
      } else {
         ...
         if (!category)
            return client.room.sendMessage(
               `Veuillez spécifier une catégorie parmi: ${Object.entries(
                  CATEGORY_SHORTCUT
               )
                  .map(
                     ([shortcut, category]) =>
         ...
         client.room.sendMessage(
            'Le mode entraînement est activé, les scores ne seront pas enregistrés.',
            'success'
         );
      }
   }
