   ...
   (client, message) => {
      ...
      if (ruleOptions.length !== 5 && ruleOptions.length !== 1)
         return client.room.sendMessage(
            'Veuillez spécifier les 5 paramètres nécessaires, ou "reset".',
            'danger'
         );
      ...
      if (ruleOptions.length === 1) {
         ...
         if (ruleValues.some((v) => Number.isNaN(v)))
            return client.room.sendMessage(
               'Veuillez spécifier des nombres valides.',
               'danger'
            );
         ...
         if (
            ...
            return client.room.sendMessage(
               'La difficulté des syllabes doit être comprise entre -5000 et 5000.',
               'danger'
            );

         if (rules.minTurnDuration < 1 || rules.minTurnDuration > 10)
            return client.room.sendMessage(
               'La durée minimum des tours doit être comprise entre 1 et 10.',
               'danger'
            );

         if (rules.maxPromptAge < 1 || rules.maxPromptAge > 16)
            return client.room.sendMessage(
               "L'âge maximum des syllabes doit être compris entre 1 et 16.",
               'danger'
            );

         if (rules.startingLives < 1 || rules.startingLives > 5)
            return client.room.sendMessage(
               'Le nombre de vies de départ doit être compris entre 1 et 5.',
               'danger'
            );

         if (rules.maxLives < 1 || rules.maxLives > 10)
            return client.room.sendMessage(
               'Le nombre maximum de vies doit être compris entre 1 et 10.',
               'danger'
            );
